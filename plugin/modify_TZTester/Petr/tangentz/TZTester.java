...
//        Code.append("\tpublic:\n");
         ...
         */
        Code.append("\tpublic void run_test(int Case) { ");
...
//        Code.append("\tprivate:\n");
    ...
    private void generate_verification_code(StringBuffer Code, DataType ReturnType)
        ...
        Code.append("\tprivate void verify_case(int Case, " + TypeString + " Expected, " + TypeString + " Received) {\n");

        Code.append("\t\tConsole.Write(\"Test Case #\" + Case + \"...\");\n");

        // Print "PASSED" or "FAILED" based on the result
        if (ReturnType.getDimension() == 0)
          Code.append("\t\tif (Expected == Received) \n");
        else
          Code.append("\t\tif (equal_arrays(Expected, Received)) \n");
        Code.append("\t\t\tConsole.WriteLine(\"PASSED\"); \n");
        Code.append("\t\telse { \n");
        Code.append("\t\t\tConsole.WriteLine(\"FAILED\"); \n");

        if (ReturnType.getDimension() == 0)
            {
            Code.append("\t\t\tConsole.WriteLine(\"\\tExpected: \\\"\" + Expected + \'\\\"\');\n");
            Code.append("\t\t\tConsole.WriteLine(\"\\tReceived: \\\"\" + Received + \'\\\"\'); } }\n");
            }
        else
            {
            Code.append("\t\t\tConsole.WriteLine(\"\\tExpected: \" + print_array(Expected)); \n");
            Code.append("\t\t\tConsole.WriteLine(\"\\tReceived: \" + print_array(Received)); } }\n");

        // Generate the vector output function
        Code.append("\tstring print_array(" + TypeString+" V) {\n");
        Code.append("\t\tSystem.Text.StringBuilder builder = new System.Text.StringBuilder();\n");
        Code.append("\t\tbuilder.Append(\"{ \");\n");
        Code.append("\t\tforeach (" + ReturnType.getBaseName().toLowerCase()+ " o in V) {\n");
        Code.append("\t\t\tbuilder.Append(\'\\\"\');\n");
        Code.append("\t\t\tbuilder.Append(o.ToString());\n");
        Code.append("\t\t\tbuilder.Append(\"\\\",\");\n");
        Code.append("\t\t}\n");
        Code.append("\t\tbuilder.Append(\" }\");\n");
        Code.append("\t\treturn builder.ToString();\n");
        Code.append("\t}\n");
        Code.append("\tbool equal_arrays(" + TypeString+" a, " + TypeString + "b) {\n");
        Code.append("\t\tif (a.Length != b.Length) return false;\n");
        Code.append("\t\tfor (int i = 0; i < a.Length; ++i) if (a[i] != b[i]) return false;\n");
    ...
    private void generate_test_case(int Index, StringBuffer Code, DataType[] ParamTypes, DataType ReturnType, TestCase Case)
        ...
        */
        Code.append("\tprivate void test_case_" + Index + "() { ");
    ...
    private void generate_parameter(int Index, StringBuffer Code, DataType ParamType, String Input)
        ...
        if (ParamType.getDimension() == 0)
            ...
            if (Desc.equals("long") || Desc.equals("Long"))
                Code.append(ParamType.getDescriptor(m_Language) + " Arg" + Index + " = " + Input + "l; ");
            else
            ...
            Code.append(ParamType.getBaseName().toLowerCase() + "[] Arg" + Index + " = new " + ParamType.getBaseName().toLowerCase() + "[]" + Input + "; ");
            }
        // end if
        }
